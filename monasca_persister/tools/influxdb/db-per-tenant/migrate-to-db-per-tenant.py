...
def usage():
    usage = """Monasca InfluxDB tool for migration to database per tenant
    ...
    migrate-to-db-per-tenant.py [-h] --config-file <ini>
        -h --help           Prints this
        --config-file <ini> (Required) Configuration file as described in README.rst

    Example:

    migrate-to-db-per-tenant.py --config-file <path>/persister.ini
...
class MigrationHelper(object):
    ...
    def _migrate(self, measurement, tenant_id, start_time_offset,
        ...
        if db_per_tenant:
            target_db = "%s_%s" % (self.conf.influxdb.database_name, tenant_id)
    ...
    def migrate(self,
        ...
        for i, measurement in enumerate(todo):
            ...
            try:
                for tenant_id in tenancy.get(measurement):
                    start_time_offset = tenant_defaults.get(
                        tenant_id, {}).get('start', default_start_time_offset)
                    end_time_offset = tenant_defaults.get(
                        tenant_id, {}).get('end', default_end_time_offset)
            ...
            LOG.info("{}/{} (done {} + skip {} + fail {})/{}".format(
                i + 1, len(todo), len(done), len(skip), len(fail),
                len(measurements)))
...
def main():
    ...
    tenant_defaults = {
        # e.g. "03cc1b94463c44e791222cb82008b8b1": rp_2w,
    }
    skip_functions = []  # e.g. [lambda x: x.startswith('log.')]

    helper = MigrationHelper()
    helper.migrate(skip_functions=skip_functions,
                   time_unit=unit,
